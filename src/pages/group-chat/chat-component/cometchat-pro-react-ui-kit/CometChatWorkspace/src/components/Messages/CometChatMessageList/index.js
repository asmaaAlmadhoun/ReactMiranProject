  ...
  render() {
    ...
    if (this.props.messages.length === 0) {
      messageContainer = (
        <div style={decoratorMessageStyle()} className="messages__decorator-message">
          <p style={decoratorMessageTxtStyle(this.props)} className="decorator-message">{this.decoratorMessage}</p>
        </div>
      );
    }
    ...
    const messages = this.props.messages.map((message, key) => {
      ...
      if (cDate !== messageSentDate) {
        dateSeparator = (
          <div style={messageDateContainerStyle()} className="message__date">
            <span style={messageDateStyle(this.props)}>{getMessageDate(dateField, this.props.lang)}</span>
          </div>);
      }
    ...
    return (
      <div className="chat__list" style={chatListStyle(this.props)}>
        {messageContainer}
        <div className="list__wrapper" style={listWrapperStyle()} ref={(el) => { this.messagesEnd = el; }} onScroll={this.handleScroll}>
          {messages}
        </div>
      </div>
    );
