...
class CometChatMessageHeader extends React.Component {
  ...
  render() {
    ...
    let status = (
      <span style={chatStatusStyle(this.props, this.state)} className={chatStatusClassName}>{this.state.status}</span>
    );

    const audioCallText = Translator.translate("AUDIO_CALL", this.props.lang);
    let audioCallBtn = ( 
      <div className={audioCallClassName} title={audioCallText} onClick={() => this.props.actionGenerated("audioCall")} style={chatOptionStyle(audioCallIcon)}>
        <img src={audioCallIcon} alt={audioCallText} />
      </div>);
    
    const videoCallText = Translator.translate("VIDEO_CALL", this.props.lang);
    let videoCallBtn = (
      <div className={videoCallClassName} title={videoCallText} onClick={() => this.props.actionGenerated("videoCall")} style={chatOptionStyle(videoCallIcon)}>
        <img src={videoCallIcon} alt={videoCallText} />
      </div>);

    const viewDetailText = Translator.translate("VIEW_DETAIL", this.props.lang);
    let viewDetailBtn = (<div className={viewDetailClassName} title={viewDetailText}  onClick={() => this.props.actionGenerated("viewDetail")} style={chatOptionStyle(detailPaneIcon)}>
    ...
    return (
      <div style={chatHeaderStyle(this.props)} className="chat__header">
        <div style={chatDetailStyle()} className="chat__details">
          <div style={chatSideBarBtnStyle(menuIcon, this.props)} className="chat__sidebar-menu" onClick={() => this.props.actionGenerated("menuClicked")}></div>
          <div style={chatThumbnailStyle()} className="chat__thumbnail">
            {avatar}
            {presence}
          </div>
          <div style={chatUserStyle()} className={chatWithClassName}>
            <h6 style={chatNameStyle()} className={chatNameClassName}
