...
class CometChatConversationListWithMessages extends React.Component {
  ...
  render() {
    ...
    if(this.state.threadmessageview) {
      threadMessageView = (
        <div style={chatScreenSecondaryStyle(this.props)} className="chats__secondary-view">
    ...
    return (
      <div style={chatScreenStyle(this.props.theme)} className="cometchat cometchat--chats" dir={Translator.getDirection(this.state.lang)}>
        <div style={chatScreenSidebarStyle(this.state, this.props)} className="chats__sidebar">
        ...
        <div style={chatScreenMainStyle(this.state, this.props)} className="chats__main">{messageScreen}</div>
        {detailScreen}
        {threadMessageView}
        {imageView}
