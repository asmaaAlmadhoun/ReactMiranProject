    ...
    render() {
        ...
        const template = (message, key) => {
            ...
            if(this.state.messagetype === "image" && message.data.url) {
                ...
                return (
                    <div id={message.id} key={key} style={itemStyle(this.state, this.props, fileIcon)} className="item item__image">
                ...
                return (
                    <div id={message.id} key={key} style={itemStyle(this.state, this.props, fileIcon)} className="item item__video">
                ...
                return (
                    <div id={message.id} key={key} style={itemStyle(this.state, this.props, fileIcon)} className="item item__file">
        ...
        return (
            <div style={sectionStyle(this.props)} className="section section__sharedmedia">
                <h6 style={sectionHeaderStyle(this.props)} className="section__header">{Translator.translate("SHARED_MEDIA", this.props.lang)}</h6>
                <div style={sectionContentStyle(this.props)} data-id="sharedmedia" className="section__content">
                    <div style={mediaBtnStyle()} className="media__button">
                        <span style={buttonStyle(this.state, "image")} onClick={() => this.mediaClickHandler("image")}>{Translator.translate("PHOTOS", this.props.lang)}</span>
                        <span style={buttonStyle(this.state, "video")} onClick={() => this.mediaClickHandler("video")}>{Translator.translate("VIDEOS", this.props.lang)}</span>
                        <span style={buttonStyle(this.state, "file")} onClick={() => this.mediaClickHandler("file")}>{Translator.translate("DOCS", this.props.lang)}</span>
                    </div>
                    <div style={mediaItemStyle()} className="media_items"
