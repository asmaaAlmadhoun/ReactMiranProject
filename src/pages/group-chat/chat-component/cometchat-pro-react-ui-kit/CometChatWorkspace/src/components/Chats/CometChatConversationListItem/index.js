...
class CometChatConversationListItem extends React.Component {
  ...
  render() {
    ...
    if (this.state.lastMessage) {
      lastMessageTimeStamp = (
        <span style={itemLastMsgTimeStyle(this.props)} className="item__details__timestamp">{this.state.lastMessageTimestamp}</span>
      );
    }
    ...
    return (
      <div style={listItem(this.props)} className="list__item" onClick={() => this.props.handleClick(this.props.conversation, this.props.conversationKey)}>
        <div style={itemThumbnailStyle()} className="list__item__thumbnail">
          {avatar}
          {presence}
        </div>
        <div style={itemDetailStyle()} className="list__item__details" dir={Translator.getDirection(this.props.lang)}>
          <div style={itemRowStyle()} className="item__details_block_one">
            <div style={itemNameStyle()} className="item__details__name"
            onMouseEnter={event => this.toggleTooltip(event, true)}
            onMouseLeave={event => this.toggleTooltip(event, false)}>{this.props.conversation.conversationWith.name}</div>
            {lastMessageTimeStamp}
          </div>
          <div style={itemRowStyle()} className="item__details_block_two">
            <div style={itemLastMsgStyle(this.props)} className="item__details__last-message"
            onMouseEnter={event => this.toggleTooltip(event, true)}
