...
class CometChatUserList extends React.PureComponent {
  ...
  render() {
    ...
    if(this.state.userlist === null) {
      messageContainer = (
        <div style={contactMsgStyle()} className="contacts__decorator-message">
          <p style={contactMsgTxtStyle(this.props)} className="decorator-message">{this.decoratorMessage}</p>
        </div>
      );
    }
    ...
    const users = userList.map((user, key) => {
      ...
      if (chr !== currentLetter) {
        currentLetter = chr;
        firstChar = (<div style={contactAlphabetStyle()} className="contacts__list__alphabet-filter">{currentLetter}</div>);
      } else {
        firstChar = null;
      }
    ...
    let closeBtn = (<div style={contactHeaderCloseStyle(navigateIcon, this.props)} className="header__close" onClick={this.handleMenuClose}></div>);
    ...
    return (
        ...
        {/*  <div></div>*/}
        {/*</div>*/}
        <div style={contactSearchStyle()} className="contacts__search">
          <input
          type="text" 
          autoComplete="off" 
          style={contactSearchInputStyle(this.props, searchIcon)}
          className="search__input" 
          placeholder={Translator.translate("SEARCH", this.state.lang)}
          onChange={this.searchUsers} />
        </div>
        {messageContainer}
        <div style={contactListStyle()} className="contacts__list" onScroll={this.handleScroll} ref={el => this.userListRef = el}>{users}</div>
      </div>
    );
  }
}
