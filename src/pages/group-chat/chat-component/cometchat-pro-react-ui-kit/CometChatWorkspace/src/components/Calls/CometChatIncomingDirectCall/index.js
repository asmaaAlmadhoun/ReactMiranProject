...
class CometChatIncomingDirectCall extends React.PureComponent {
    ...
    render() {
        ...
        if (this.state.incomingMessage) {
            ...
            if (this.state.incomingMessage) {
                avatar = (
                    <div style={thumbnailStyle()} className="header__thumbnail">
            ...
            messageScreen = (
                <div style={incomingCallWrapperStyle(this.props, keyframes)} className="callalert__wrapper">
                    <div style={callContainerStyle()} className="callalert__container">
                        <div style={headerWrapperStyle()} className="callalert__header">
                            <div style={callDetailStyle()} className="header__detail">
                                <div style={nameStyle()} className="name">{this.state.incomingMessage.sender.name}</div>
                                <div style={callTypeStyle(this.props)} className="calltype">{callType}</div>
                            </div>
                            {avatar}
                        </div>
                        <div style={headerButtonStyle()} className="callalert__buttons">
                            <button type="button" style={ButtonStyle(this.props, 0)} className="button button__ignore" onClick={this.ignoreCall}>{Translator.translate("IGNORE", this.props.lang)}</button>
                            <button type="button" style={ButtonStyle(this.props, 1)} className="button button__join" onClick={this.joinDirectCall}>{Translator.translate("JOIN", this.props.lang)}</button>
                        </div>
                    </div>
                </div>
            );
