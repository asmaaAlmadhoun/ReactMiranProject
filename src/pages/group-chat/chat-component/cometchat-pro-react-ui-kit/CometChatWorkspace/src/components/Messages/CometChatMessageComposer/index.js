...
class CometChatMessageComposer extends React.PureComponent {
  ...
  getConversationId = () => {
    ...
    if (this.props.type === CometChat.RECEIVER_TYPE.USER) {
      ...
      const users = [localStorage.getItem('chat_uid'), this.props.item.uid];
  ...
  render() {
    ...
    if (enums.LIVE_REACTIONS.hasOwnProperty(this.props.reaction)) {
      ...
      liveReactionBtn = (
        <div title={liveReactionText} style={reactionBtnStyle(imgSrc)} className="button__reactions" onClick={this.sendReaction}>
          <img src={enums.LIVE_REACTIONS['heart']} alt={reactionName} />
        </div>
      );
    }
    ...
    let avp = (
      <React.Fragment>
        <div title={videoText} style={fileItemStyle()} className="filelist__item item__video" onClick={() => { this.openFileDialogue("video") }}>
          <img src={videoIcon} alt={videoText} />
          <input onChange={this.onVideoChange} accept="video/*" type="file" ref={this.videoUploaderRef} />
        </div>
        <div title={audioText} style={fileItemStyle()} className="filelist__item item__audio" onClick={() => { this.openFileDialogue("audio") }}>
          <img src={audioIcon} alt={audioText} />
          <input onChange={this.onAudioChange} accept="audio/*" type="file" ref={this.audioUploaderRef} />
        </div>
        <div title={imageText} style={fileItemStyle()} className="filelist__item item__image" onClick={() => { this.openFileDialogue("image") }}>
    ...
    let sendBtn = (
      <div title={sendMessageText} style={sendButtonStyle()} className="button__send" onClick={this.sendTextMessage}>
    ...
    let attach = (
      <div style={stickyAttachmentStyle()} className="input__sticky__attachment">
        <div style={stickyAttachButtonStyle()} className="attachment__icon" onClick={this.toggleFilePicker} title={attachText}>
          <img src={roundedPlus} alt={attachText} />
        </div>
        <div style={filePickerStyle(this.state)} className="attachment__filepicker" dir={Translator.getDirection(this.props.lang)}>
          <div style={fileListStyle()} className="filepicker__filelist">
            {avp}
            {docs}
            {createPollBtn}
            {collaborativeDocBtn}
    ...
    if (this.state.messageToBeEdited) {
      ...
      editPreview = (
        <div style={editPreviewContainerStyle(this.props, keyframes)}>
          <div style={previewHeadingStyle()}>
            <div style={previewTextStyle()}>{Translator.translate("EDIT_MESSAGE", this.props.lang)}</div>
            <span style={previewCloseStyle(closeIcon)} onClick={this.closeEditPreview}></span>
          </div>
          <div>{messageText}</div>
        </div>
      );
    ...
    return (
      <div style={chatComposerStyle(this.props)} className="chat__composer">
        {editPreview}
        {smartReplyPreview}
        {stickerViewer}
        {emojiViewer}
        <div style={composerInputStyle()} className="composer__input">
          <div tabIndex="-1" style={inputInnerStyle(this.props, this.state)} className="input__inner">
            <div
            style={messageInputStyle(disabledState)}
            ...
            <div style={inputStickyStyle(this.props, disabledState)} className="input__sticky">
              {attach}
              <div style={stickyButtonStyle(this.props, this.state)} className="input__sticky__buttons">
                {stickerBtn}
                {emojiBtn}
                {sendBtn}
                {liveReactionBtn}
