    ...
    render() {
        ...
        if (this.state.activestickerlist.length === 0) {
            messageContainer = (
                <div style={stickerMsgStyle()} className="stickers__decorator-message">
                    <p style={stickerMsgTxtStyle(this.props.theme)} className="decorator-message">{this.decoratorMessage}</p>
                </div>
            );
        }
        ...
        if (Object.keys(this.state.stickerset).length) {
            ...
            const sectionItems = Object.keys(this.state.stickerset).map((sectionItem, key) => {
                ...
                style={sectionListItemStyle(this.props)}
            ...
            if (this.state.activestickerlist.length) {
                ...
                activeStickerList = stickerList.map((stickerItem, key) => {
                    ...
                    return (
                        <div key={key} style={stickerItemStyle(this.props)} onClick={() => this.sendStickerMessage(stickerItem)} className="stickers__listitem">
            ...
            stickers = (
                <React.Fragment>
                    <div style={stickerCloseStyle(closeIcon)} className="stickers__close" onClick={() => this.props.actionGenerated("closeSticker")}></div>
                    <div style={stickerListStyle(this.props)} className="stickers__list">{activeStickerList}</div>
                    <div style={stickerSectionListStyle(this.props)} className="stickers__sections">{sectionItems}</div>
                </React.Fragment>
            );
        }

        return (
            <div style={stickerWrapperStyle(this.props, keyframes)} className="stickers">
                {messageContainer}
                {stickers}
            </div>
        );
