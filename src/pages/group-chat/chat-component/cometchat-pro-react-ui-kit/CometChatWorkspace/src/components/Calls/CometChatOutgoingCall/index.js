...
class CometChatOutgoingCall extends React.PureComponent {
  ...
  render() {
    ...
    if (this.state.callInProgress) {
      ...
      if (this.state.errorScreen) {
        errorScreen = (
          <div style={errorContainerStyle()} className="callscreen__error__wrapper"><div>{this.state.errorMessage}</div></div>
        );
      }

      if (this.state.outgoingCallScreen) {
        outgoingCallScreen = (
          <div style={callScreenContainerStyle()} className="callscreen__container">
            <div style={headerStyle()} className="callscreen__header">
              <span style={headerDurationStyle()} className="header__calling">{Translator.translate("CALLING", this.props.lang)}</span>
              <h6 style={headerNameStyle()} className="header__name">{this.state.callInProgress.receiver.name}</h6>
            </div>
            <div style={thumbnailWrapperStyle()} className="callscreen__thumbnail__wrapper">
              <div style={thumbnailStyle()} className="callscreen__thumbnail">{avatar}</div>
            </div>
            {errorScreen}
            <div style={headerIconStyle()} className="callscreen__icons">
              <div style={iconWrapperStyle()} className="icon__block" onClick={this.cancelCall}>
                <div style={iconStyle(callIcon, 0)} className="icon icon__end"></div>
              </div>
            </div>
          </div>
        );
    ...
    if (this.state.callInProgress) {
      ...
      callScreen = (
        <div style={callScreenWrapperStyle(this.props, keyframes)} className="callscreen__wrapper" ref={el => { this.callScreenFrame = el; }}>
          {outgoingCallScreen}
        </div>
      );
    }
