...
class CometChatConversationList extends React.Component {
  ...
  render() {
    ...
    if(this.state.conversationlist.length === 0) {
      messageContainer = (
        <div style={chatsMsgStyle()} className="chats__decorator-message">
          <p style={chatsMsgTxtStyle(this.props)} className="decorator-message">{this.decoratorMessage}</p>
        </div>
      );
    }

    let closeBtn = (<div style={chatsHeaderCloseStyle(navigateIcon, this.props)} className="header__close" onClick={this.handleMenuClose}></div>);
    if (!this.props.hasOwnProperty("enableCloseMenu") || (this.props.hasOwnProperty("enableCloseMenu") && this.props.enableCloseMenu === 0)) {
      closeBtn = null;
    }    

    return (
      <div style={chatsWrapperStyle()} className="chats">
        <div style={chatsHeaderStyle(this.props)} className="chats__header">
          {closeBtn}
          <h4 style={chatsHeaderTitleStyle(this.props)} className="header__title" dir={Translator.getDirection(this.state.lang)}>{Translator.translate("CHATS", this.state.lang)}</h4>
        </div>
        {messageContainer}
        <div style={chatsListStyle()} className="chats__list" onScroll={this.handleScroll} ref={el => this.chatListRef = el}>{conversationList}</div>
      </div>
    );
  }
}
